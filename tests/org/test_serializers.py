...
class TeamSerializerTestCase(TestCase):
    def setUp(self):
        ...
        self.director = User.objects.create_user(username='z-director', password='secret',
                                                 date_joined=datetime.datetime(2019, 1, 1, tzinfo=pytz.UTC))
        self.backend = User.objects.create_user(username='backend', password='secret',
                                                date_joined=datetime.datetime(2019, 1, 1, tzinfo=pytz.UTC))
        self.director_member = Member.objects.create(student=self.director.student, year_joined=datetime.date.today(),
                                                     team=self.team, url='director')
        self.director_member.roles.add(self.director_role)
        self.backend_member = Member.objects.create(student=self.backend.student, year_joined=datetime.date.today(),
