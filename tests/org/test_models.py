...
class TeamTestCase(TestCase):
    def setUp(self):
        Team.objects.create(name="Directors", description="Important people", order=1)
        Team.objects.create(name="Platform", description="Important stuff", order=2)
        Team.objects.create(name="Android", description="Important information", order=3)
...
class MemberTestCase(TestCase):
    def setUp(self):
        self.member = get_user_model().objects.create_user(
            pennid=1, username="member", password="secret"
        )
